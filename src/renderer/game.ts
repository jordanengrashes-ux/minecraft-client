...
// ── World storage ──────────────────────────────────────────────────────────────
const worldData = new Map<string, number>(); // "x,y,z" -> blockId
...
function getBlock(x:number,y:number,z:number):number {
  ...
  if (y > h) return 0;
  if (y === h) return 3;       // grass
  if (y >= h-3) return 2;      // dirt
  return 1;                    // stone
}
...
function rebuildChunk(cx:number, cz:number) {
  ...
  for (let lx=0;lx<CHUNK;lx++) for (let lz=0;lz<CHUNK;lz++) for (let y=0;y<WORLD_HEIGHT;y++) {
    ...
    for (const [dx,dy,dz] of faces) {
      if (getBlock(wx+dx,y+dy,wz+dz) !== 0) continue; // face hidden
  ...
  // Merge all faces into one geometry with groups per material
  const allPos: number[] = [], allNrm: number[] = [], allIdx: number[] = [];
  ...
  for (const [id, verts] of groups) {
    ...
    const quadCount = verts.length / (4*6);
    for (let q=0;q<quadCount;q++) {
      allIdx.push(vBase,vBase+1,vBase+2,vBase,vBase+2,vBase+3);
      vBase += 4;
    }
    for (const v of verts) (v < 100 ? allPos : allNrm).push(); // won't run — placeholder
    mats.push(getMat(def.color, id===6));
    // suppress unused warning
    void idxStart;
  }

  // Simpler: one mesh per block type in chunk
  ...
  for (const [id, verts] of groups) {
    ...
    geo.setIndex(idx);
    const mesh = new THREE.Mesh(geo, getMat(def.color, id===6));
    mesh.castShadow = true; mesh.receiveShadow = true;
    group.add(mesh);
  }
  // Store as single object per chunk (group)
  (group as any)._isChunkGroup = true;
  scene.add(group);
  chunkMeshes.set(key, group as unknown as THREE.Mesh);
}

// face vertex helper: returns [pos x4, nrm x4] flat
...
sun.position.set(50,120,30); sun.castShadow=true;
scene.add(sun);
...
const SPEED  = 5, JUMP = 9, GRAVITY = 22;
...
window.addEventListener('keydown', e => {
  ...
  if (e.code==='KeyT') { e.preventDefault(); openChatInput(); }
...
function closeChatInput() {
  chatOpen=false; chatInput.style.display='none'; chatInput.value='';
}
function sendChat() {
...
window.addEventListener('wheel', e => {
  setHotbar((hotbarSlot + (e.deltaY>0?1:-1) + 9) % 9);
});
...
function raycastBlock(origin:THREE.Vector3, dir:THREE.Vector3, maxDist:number) {
  ...
  for (let d=0;d<maxDist;d+=step) {
    ...
    if (getBlock(bx,by,bz)!==0) {
      return {x:bx,y:by,z:bz, nx:bx-lastX,ny:by-lastY,nz:bz-lastZ};
    }
...
function updatePhysics(dt:number) {
  ...
  // Move + collide
  pos.y += vel.y*dt;
  const foot = Math.floor(pos.y-1.8);
  const head = Math.floor(pos.y+0.1);
  const bx=Math.floor(pos.x), bz=Math.floor(pos.z);
  if (vel.y<0 && getBlock(bx,foot,bz)!==0) {
    pos.y=foot+1+1.8; vel.y=0; onGround=true;
  } else if (vel.y>0 && getBlock(bx,head,bz)!==0) {
    pos.y=head-0.1; vel.y=0;
  } else { onGround=false; }

  pos.x+=vel.x*dt;
  if (getBlock(Math.floor(pos.x),Math.floor(pos.y-0.9),bz)!==0 ||
      getBlock(Math.floor(pos.x),Math.floor(pos.y-1.7),bz)!==0) { pos.x-=vel.x*dt; }

  pos.z+=vel.z*dt;
  if (getBlock(bx,Math.floor(pos.y-0.9),Math.floor(pos.z))!==0 ||
      getBlock(bx,Math.floor(pos.y-1.7),Math.floor(pos.z))!==0) { pos.z-=vel.z*dt; }
}
...
function updateChunks() {
  ...
  for (const k of [...loadedChunks]) {
    ...
    if (Math.abs(kcx-cx)>RENDER_DIST+1||Math.abs(kcz-cz)>RENDER_DIST+1) {
      ...
      if (m) { scene.remove(m); chunkMeshes.delete(k); }
...
function initMultiplayer() {
  const myRef = ref(rtdb,`voxel/players/${sessionId}`);
  ...
  onValue(ref(rtdb,'voxel/players'), snap => {
    ...
    if (data) for (const [id,s] of Object.entries(data)) {
      ...
      seen.add(id);
      if (!remotePlayers.has(id)) addRemotePlayer(id,s);
      else updateRemotePlayer(id,s);
    }
  ...
  onValue(ref(rtdb,'voxel/blocks'), snap => {
    ...
    for (const [k,v] of Object.entries(data)) {
      ...
      worldData.set(k,v as number);
      rebuildChunk(Math.floor(x/CHUNK),Math.floor(z/CHUNK));
    }
  });

  const chatRef = ref(rtdb,'voxel/chat');
  onValue(chatRef, snap => {
    ...
    for (const m of msgs as any[]) {
      ...
      d.textContent=`<${m.name}> ${m.msg}`; log.appendChild(d);
    }
...
function addRemotePlayer(id:string,s:any) {
  ...
  label.textContent=s.name||'Player';
  document.body.appendChild(label);
  mesh.position.set(s.x||0,s.y||0,s.z||0);
  remotePlayers.set(id,{mesh,label});
}

function updateRemotePlayer(id:string,s:any) {
  const rp=remotePlayers.get(id)!;
  rp.mesh.position.set(s.x||0,s.y||0,s.z||0);
  rp.label.textContent=s.name||'Player';
}

function removeRemotePlayer(id:string) {
  const rp=remotePlayers.get(id); if(!rp) return;
  scene.remove(rp.mesh); rp.label.remove(); remotePlayers.delete(id);
}
...
function updateHUD(dt:number) {
  ...
  (document.getElementById('info-player') as HTMLElement).textContent=`Player: ${playerName}  ${flying?'[FLY]':''}`;
}

// ── Init ───────────────────────────────────────────────────────────────────────
function init(data:{uid:string;name:string}) {
  uid=data.uid; playerName=data.name;
  (document.getElementById('info-player') as HTMLElement).textContent=`Player: ${playerName}`;

  // Place spawn on ground
...
// Receive user data from Electron main or sessionStorage fallback
...
function loop(now=0) {
  ...
  // Multiplayer publish
  publishTimer+=dt;
  if (publishTimer>0.1){publishTimer=0;publishPlayer();}
